...
https://github.com/shenlebantongying/Comment-Print-Test
"""
...
if __name__ == "__main__":
    ...
    args_parser.add_argument(
        "-s", "--source", type=str, required=True, help="Source code to Scan"
    )

    args_parser.add_argument(
        "-c", "--command", type=str, required=True, help="Command to run"
    )
    ...
    for line_number, line in zip(count(0), source_file):
        ...
        if p is not None:
            print(
                r"[{a}/{b}]=>".format(a=test_number + 1, b=test_total),
                source_file[line_number + 1].strip(),
            )
