...
(fiveam:test all
  (fiveam:is (equal (inc-one '(1 2 3)) '(2 3 4))))
...
(fiveam:test apply
  (fiveam:is (equal (apply #'+ '(1 2))
                    (apply (function +) '(1 2)))))

(defun bad-list-rev (L)
  "recrusively reverse a list"
  (if (null L)
      nil
      (append (bad-list-rev (rest L))
              (list (first L)))))

(fiveam:test list-rec-rev
  (fiveam:is (equal (bad-list-rev '(1 2 3))
                    '(3 2 1))))
