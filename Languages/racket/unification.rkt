...
(define (occurs? u v)
  (and (pair? v)
       (let f ([l (cdr v)])
         (and (pair? l) (or (eq? u (car l)) (occurs? u (car l)) (f (cdr l)))))))
...
(define uni
  (lambda (u v s ks kf)
    (cond
      ...
      [(and (eq? (car u) (car v)) (= (length u) (length v)))
       (let f ([u (cdr u)]
               [v (cdr v)]
               [s s])
         (if (null? u)
...
(define unify (lambda (u v) (uni u v (lambda (x) x) (lambda (s) (s u)) (lambda (msg) msg))))
