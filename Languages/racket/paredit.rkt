    ...
    [(_ key (name . args) body* ...)
     #'(define-shortcut key
                        ...
                        (λ args
                          body* ...))]
    [(_ (key ...) name proc)
     #`(define-shortcut-internal (#,@(add-esc-and-option-key-bindings #'(key ...))) name proc)]
    [(_ key name proc) #'(define-shortcut (key) name proc)]))
...
#;(define (get-paredit-forward-sexp ed sp)
    (cond
      ...
      [(send ed find-up-sexp sp)
       =>
       (λ (pos) (send ed get-forward-sexp pos))]
      [else #f]))

#;(define-shortcut ("c:m:f")
                   (paredit-forward-sexp ed evt)
                   (define sp (send ed get-start-position))
                   (and-let* ([dest (get-paredit-forward-sexp ed sp)]) (send ed set-position dest)))
...
#;(define-shortcut ("c:m:b")
                   (paredit-backward-sexp ed evt)
                   (define sp (send ed get-start-position))
                   (and-let* ([dest (get-paredit-backward-sexp ed sp)]) (send ed set-position dest)))

#;(define-shortcut ("c:m:d")
                   (paredit-down-sexp ed evt)
                   (send ed down-sexp (send ed get-start-position)))

#;(define (get-forward-atom ed pos)
    (define sp (send ed get-start-position))
    (define dests
      (filter (λ (x) x) (list (send ed find-down-sexp sp) (get-paredit-forward-sexp ed sp))))
    (and (not (null? dests)) (apply min dests)))

#;(define-shortcut ("m:right")
                   (forward-atom ed evt)
...
#;(define (find-down-sexp-backward ed pos)
    (and-let* ([bw (send ed get-backward-sexp pos)] [down (send ed find-down-sexp bw)])
...
#;(define (get-backward-atom ed pos)
    (define sp (send ed get-start-position))
    (define dests
      (filter (λ (x) x) (list (find-down-sexp-backward ed sp) (get-paredit-backward-sexp ed sp))))
    (and (not (null? dests)) (apply max dests)))

#;(define-shortcut ("m:left")
                   (backward-atom ed evt)
                   (and-let* ([dest (get-backward-atom ed (send ed get-start-position))])
                             (send ed set-position dest)))

;;; Depth-Changing
(define-shortcut ("m:s")
                 (paredit-splice-sexp ed evt [pos #f] [reindent #t])
                 (when (not pos)
...
#;(define-shortcut ("m:up")
                   (paredit-splice-sexp-killing-backward ed evt)
                   (define sp (sexp-start ed))
                   (when (not-toplevel? ed sp)
                     (kill-sexps-backward ed sp)
                     (paredit-splice-sexp ed evt)))

#;(define-shortcut ("m:down")
                   (paredit-splice-sexp-killing-forward ed evt)
                   (define sp (sexp-start ed))
                   (when (not-toplevel? ed sp)
...
(define-shortcut ("m:s")
                 (paredit-raise-sexp ed evt)
                 (define sp (sexp-start ed))
                 (when (not-toplevel? ed sp)
                   (and-let* ([fw (send ed get-forward-sexp sp)]) (kill-sexps-forward ed fw))
                   (kill-sexps-backward ed sp)
                   (paredit-splice-sexp ed evt)))

#;(define-shortcut ("m:?")
                   (paredit-convolute-sexp ed evt)
                   (define sp (sexp-start ed))
                   (and-let* ([r1 (send ed find-up-sexp sp)] [fw (send ed get-forward-sexp r1)]
...
(define (find-up-sexp-slurp-forward ed sp)
  (let loop ([sp (send ed find-up-sexp sp)])
    (cond
      [(not sp) #f]
      [(and-let* ([fw1 (send ed get-forward-sexp sp)]) (send ed get-forward-sexp fw1)) sp]
      [else (loop (send ed find-up-sexp sp))])))

(define-shortcut
 ("c:right" "c:s:0" "c:]")
 (paredit-slurp-forward ed evt)
 (define sp (send ed get-start-position))
 (and-let* ([up (find-up-sexp-slurp-forward ed sp)] [end (send ed get-forward-sexp up)]
...
(define-shortcut
 ("c:m:left" "c:s:9" "c:[")
 (paredit-slurp-backward ed evt)
 (define sp (send ed get-start-position))
 (and-let* ([start (find-up-sexp-slurp-backward ed sp)] [bw (send ed get-backward-sexp start)]
...
(define-shortcut
 ("c:left" "c:}")
 (paredit-barf-forward ed evt)
 (define sp (send ed get-start-position))
 (and-let* ([up (send ed find-up-sexp sp)] [fw (send ed get-forward-sexp up)]
                                           [paren (send ed get-text (- fw 1) fw)]
                                           [last (last-sexp ed sp)]
                                           [bw (send ed get-backward-sexp last)])
           (and-let* ([bw1 (send ed get-backward-sexp bw)] [x (send ed get-forward-sexp bw1)])
                     (set! bw x))
           (send ed delete fw)
           (send ed insert paren bw)
...
(define-shortcut
 ("c:m:right" "c:{")
 (paredit-barf-backward ed evt)
 (define sp (send ed get-start-position))
 (and-let* ([up (send ed find-up-sexp sp)] [paren (send ed get-text up (+ up 1))]
                                           [down (send ed find-down-sexp up)]
                                           [fw (send ed get-forward-sexp down)])
           (and-let* ([fw1 (send ed get-forward-sexp fw)] [x (send ed get-backward-sexp fw1)])
                     (set! fw x))
           (send ed insert paren fw)
           (send ed delete (+ up 1))
           (send ed tabify-selection up fw)))
