...
(define (update* f xs ys)
  (match* (xs ys)
    [['() '()] f]
    [[(cons x xs*) (cons y ys*)] (update* (update f x y) xs* ys*)]))
...
(define (eval-atom atom ρ σ)
  (match atom
    ...
    [(cons (and prim (? prim?)) rest)
     ; =>
     (let ([args (map (eval-atom/curry ρ σ) rest)]) (apply (prim->proc prim) args))]
...
(define (apply-proc proc args σ κ)
  (match proc
    ...
    [`(cont ,κ*) (apply-kont κ* (car args) σ)]))
...
(define (step ς)
  (match ς
    ...
    ; let-binding:
    ; =>
    [(state `(let ([,v ,exp]) ,body) ρ σ κ) (state exp ρ σ `(letk ,v ,body ,ρ ,κ))]
